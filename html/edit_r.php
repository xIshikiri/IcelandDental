...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    $editQuery = $db -> prepare("UPDATE Receptionist SET Name = ?, Surname = ?, DateOfBirth = ?, Adress = ?, PhoneNumber = ? WHERE ID = {$id};");
...
<html lang="pl-PL">
    ...
    <h2>Edycja</h2>
        <div>
            <form method="post">
                <?php
                    if ($query = $db -> prepare("SELECT * FROM Receptionist WHERE ID = $id ;")){
                    $query -> execute();
